...
CREATE DATABASE IF NOT EXISTS `mahjongdb` DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;
...
CREATE TABLE `YAKU_DESC` (
    ...
    `desc` VARCHAR(1000),
    PRIMARY KEY (`id`),
    FOREIGN KEY (`id`) REFERENCES `HAN` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = INNODB DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;
...
CREATE TABLE `User_Post` (
    ...
    PRIMARY KEY (`write_id`) -- FOREIGN KEY (`write_user`) REFERENCES `User_Data` (`email`)
...
VALUES
    (1, '이동익', '제목임1', '2022-06-20', '내용임1'),
    (2, '정아무개', '제목임2', '2022-06-21', '내용임2'),
    (3, '김아무개', '제목임3', '2022-06-22', '내용임3'),
...
INSERT INTO
    YAKU_IMAGE (id, yaku_name, image, yaku_nick)
VALUES
...
VALUES
    ...
    (
        30,
        '유국만관',
        '아무도 화료하지 못해 유국이 될 때, 자신이 버린 패들이 전부 요구패일 경우'
    ),
    (31, '청일색', '한 가지 종류의 수패만으로 화료하는 경우'),
    (
        32,
        '구련보등',
        '한 가지 종류의 수패로 1112345678999 같은 종류 수패 아무거나 한개를 완성하는 경우. 9면대기 로 완성했다면(순정구련보등) 더블 역만 취급한다.'
    ),
    (
        33,
        '녹일색',
    ...
    (
        34,
        '사암각',
        '안커(울지 않고 만든 코쯔) 네 개로 화료하는 경우. 단기대기로 완성했다면(사암각 단기) 더블 역만 취급한다.'
    ),
    ...
    (
        37,
        '국사무쌍',
        '모든 수패의 노두패와 모든 자패를 모으고, 여기에 해당 패 하나를 더 모으는 경우.13면대기로 완성했다면 (국사무쌍13면대기) 더블 역만 취급한다.'
    ),
    ...
    (
        40,
        '대사희',
        '바람패 네 가지를 모두 코쯔나 깡쯔로 만드는 경우. 더블 역만으로 취급되는 경우도 많다.'
    ),
    ...
    (
        43,
        '지화',
        '자신이 자이고 첫 쯔모패를 가지고 오자마자 화료 형태가 완성되는 경우. 즉 배패만으로 텐파이 후, 첫 쯔모로 바로 쯔모화료할 경우'
    ),
    (44, '헤아림역만', '역들을 중첩시켜 도합 13판 이상으로 화료.');

select * from HAN;

# select HAN.yaku_name, menzen,`call`,`desc`,YAKU_IMAGE.image from HAN JOIN YAKU_DESC WHERE HAN.id = YAKU_DESC.id and YAKU_DESC.id = YAKU_IMAGE.id;

#id값없이 조인
select
    HAN.yaku_name,
    YAKU_IMAGE.yaku_nick,
    menzen,
    `call`,
    `desc`,
    YAKU_IMAGE.image
